...
const CertCard = ({ cert, onDelete }) => {
  ...
  return (
    <div className="relative group">
      <div className="absolute -inset-0.5 bg-gradient-to-r from-[#ffffff] to-[#e5e7eb] rounded-2xl blur opacity-10 group-hover:opacity-30 transition duration-500" />
      <div className="relative bg-[#141414] border border-white/20 rounded-2xl overflow-hidden">
      ...
      <Card>
        <div className="p-5 sm:p-6 space-y-4">
          <h2 className="text-sm font-semibold text-white flex items-center gap-2">
            <Plus className="w-4 h-4 text-white" /> Upload Certificate
          </h2>
          ...
          <label
            ...
            className={`flex flex-col items-center justify-center w-full min-h-[160px] rounded-xl border-2 border-dashed cursor-pointer transition-all duration-300 ${
              dragOver ? 'border-white/60 bg-white/10' : 'border-white/12 bg-white/4 hover:border-white/35 hover:bg-white/7'
            }`}
          >
            {preview ? (
              ...
              <div className="text-center space-y-2 p-6">
                <div className="w-11 h-11 rounded-full bg-white/10 border border-white/20 flex items-center justify-center mx-auto">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex gap-2 shrink-0">
                ...
                <button onClick={uploadImage} disabled={uploading} className="relative group/u">
                  <div className="absolute -inset-0.5 bg-gradient-to-r from-[#d1d5db] to-[#ffffff] rounded-xl opacity-60 blur group-hover/u:opacity-100 transition duration-300" />
                  <div className="relative flex items-center gap-2 px-4 py-1.5 bg-[#0A0A0A] rounded-xl border border-white/10">
                    {uploading ? <div className="w-3.5 h-3.5 border-2 border-white/20 border-t-white rounded-full animate-spin" /> : <Upload className="w-3.5 h-3.5 text-white" />}
      ...
      {loading ? (
        ...
        <Card>
          <div className="p-16 text-center">
            <Award className="w-10 h-10 text-white/25 mx-auto mb-3" />
            <p className="text-gray-400 text-sm font-medium">No certificates yet</p>
            <p className="text-gray-600 text-xs mt-1">Upload your first certificate above</p>
          </div>
        </Card>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
